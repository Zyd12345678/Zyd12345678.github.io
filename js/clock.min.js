...
function getIpInfo() {
	...
	"true" === clock_default_rectangle_enable && e ? fetch(`https://restapi.amap.com/v3/geocode/regeo?key=${gaud_map_key}&location=${clock_rectangle}`).then((e => e.json())).then((e => {
		...
		fetch(`https://jp436g38at.re.qweatherapi.com/v7/weather/now?location=${clock_rectangle}&key=${qweather_key}`).then((e => e.json())).then((a => {
		...
		e.qweather_url = `https://jp436g38at.re.qweatherapi.com/v7/weather/now?location=${a}&key=${qweather_key}`, Array.isArray(c.rectangle) ? fetch(`https://restapi.amap.com/v3/geocode/regeo?key=${gaud_map_key}&location=${clock_rectangle}`).then((e => e.json())).then((a => {
			if ("1" === a.status) {
